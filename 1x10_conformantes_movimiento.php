<?php
	...
	$id = base64_decode($_GET['id_movimiento']);
	$crud->sql = "SELECT id from caracterizacion where id_ms = ".$id;
	$crud->total();
	$total = $crud->total;
?>
<div class="row">
		...
		<div class="col-md-12">
			<div class="col-md-4">
				<?php
					if($total < 10)
					{
						echo "<button class='btn btn-success btn-md btn-block' data-target='#agregar_modal' data-toggle='modal' data-backdrop='static'>Agregar Integrante</button>";
					}
				?>
			</div>
			<br>
						...
						<?php
							$crud->sql = "SELECT * from caracterizacion 
										...
										INNER JOIN caracterizacion_militancia cm ON caracterizacion.cedula = cm.cedula  
										 where caracterizacion.id_ms = $id and caracterizacion_datos_al.id_ms = $id and cm.id_ms = $id";
							$crud->leer();
	...
	<div id="agregar_modal" class="modal fade" role="dialog">
	    <div class="modal-dialog" style="width: 70%">
	        <div class="modal-content">
	            ...
	            <form class="form-horizontal" id="form_estructura" action="grabar_movimientos.php" method="POST">
	                <input type="hidden" name="accion" value="grabar_conformante">
	                <input type="hidden" name="id_ms" value="<?php echo $id; ?>">
...
//----------------------------------------------
	...
	//--------------------------------------------------------------------------------------
		...
		var chart;
            ...
            var chartData = [
            <?php 
                $crud->sql = "SELECT nombre, apellido, (SELECT COUNT(*) from caracterizacion where id_ms = estructuras.id) as total from estructuras where id_movimiento = $_SESSION[id]";
                $crud->leer();
                foreach ($crud->filas as $row) 
                {
