			...
			<div class="col-md-12">
				<table class="table table-bordered table-hover table-condensed" id="tabla">
					...
					<tbody style="text-align:center;">
						<?php
	                        $crud->sql = "SELECT usuarios.id, usuario, id_estado, id_municipio,
                                            ...
                                            (SELECT municipio from municipios where id_municipio = usuarios.id_municipio and id_estado  = $_SESSION[estado]) as municipio
											FROM usuarios 
	                                        LEFT JOIN responsables ON usuarios.id = responsables.id_usuario
	                                        WHERE usuarios.nivel = 2 and usuarios.id_estado = $_SESSION[estado]
	                                        ";
							$crud->leer();
									...
									<tr>
	                                    ...
	                                    <td>
                                    		<button class="btn btn-success btn-sm" data-toggle="modal" data-target="#mod_project" data-backdrop="static"
                                                ...
                                                data-id_municipio="<?php echo $row['id_municipio']; ?>"
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header login-header">
                 ...
                 <input type="hidden" name="accion" value="municipales_agregar">
                    <div class="modal-body">
                            <div class="form-group">
                            	<div class="col-md-6">
                            		<select name="estado" id="estado" required="">
                            			<?php
                            				$crud->sql = "SELECT id , estado from estados where id = $_SESSION[estado] ORDER BY estado asc";
                            				$crud->leer();
                            	...
                            	<div class="col-md-6">
                            		<select name="municipio" id="municipio" class="form-control">
                            			<?php
                                            $crud->sql = "SELECT id_municipio , municipio from municipios where id_estado = $_SESSION[estado]";
                                            $crud->leer();
                                            foreach ($crud->filas as $value) 
                                            {?>
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header login-header">
                 ...
                 <input type="hidden" name="accion" value="municipales_modificar">
                 ...
                 <input type="hidden" name="id_responsable_modi" id="id_responsable_modi">
                    <div class="modal-body">
                    		<div class="form-group">
                            	...
                            	<div class="col-md-6">
                            		<select name="municipio_modi" id="municipio_modi" class="form-control">
                            			<?php
                            				$crud->sql = "SELECT id_municipio , municipio from municipios where id_estado = $_SESSION[estado]";
                            				$crud->leer();
                            				foreach ($crud->filas as $value) 
                            ...
                            <select name="permisos_modi" required="" id="permisos_modi">
                                <option value="1">Todo</option>
                                <option value="2">Moderado</option>
                                <option value="3">Regtringido</option>
                            </select>
...
//----------------------modales modificar--------------------------------------------
		...
		$("#mod_project").on('show.bs.modal', function(e){
			...
			var x = $(e.relatedTarget).data().usuario_mod;
				$(e.currentTarget).find('#usuario_modi').val(municipio).prop('selected', true).change();
                var x = $(e.relatedTarget).data().id_municipio;
                $(e.currentTarget).find('#municipio_modi').val(x).prop('selected', true).change();
            ...
            var x = $(e.relatedTarget).data().id_municipio;
                $(e.currentTarget).find('#municipio_modi').val(x);
            var x = $(e.relatedTarget).data().id_estado;
                $(e.currentTarget).find('#estado_modi').val(x);
	   });

// ==================================== BUSCAR MUNICIPIO ========================================================0

    function buscar_municipio(estado,campo)
    {
        var municipios = "";
        
        $.getJSON('buscar_municipio.php', {estado: estado}, function(data){
            $.grep(data, function(e,i){
                municipios += '<option value="'+e.id_municipio+'">'+e.municipio+'</option>';
            });
            $("#"+campo).html('').html(municipios);
        });
    ...
    $("#estado").change(function(){
        ...
        buscar_municipio(id,campo);
    })

    $("#estado_modi").change(function(){
        var id = $(this).val(),
            campo = "municipio_modi";        
    })
...
//-------------------------------------------------------------------------------------------------------------------------------------
		...
		$(".eliminar").click(function(e){
			...
			if(flujo)
			{
				$.post('grabar_cuentas.php', {id: x, accion: 'municipales_eliminar'}, function(){
					window.location.reload();
				});
			}
			else
        ...
        var chart;
            ...
            var data = [
                <?php 
                    ...
                    COUNT(*) as total from usuarios where categoria = 2 and id_estado = $_SESSION[estado] GROUP BY categoria
                    ORDER BY total DESC
                    ";
