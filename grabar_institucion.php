<?php
	...
	switch ($_REQUEST['accion']) 
	{
		case 'grabar_estructura':
			...
			$crud->sql = "SELECT * from estructuras where cedula = $_POST[cedula] and id_institucion_lider = $_POST[id_institucion]";
			$crud->total();
			if($crud->total > 0)
			{
			...
			else
			{
				...
				$crud->campos = "id_institucion, nac, cedula, nombre, apellido, telefono, direccion, email, fecha_nacimiento, centro_votacion, sexo";
				$crud->valores = "$_POST[id_institucion], '$_POST[nac]','$_POST[cedula]','$_POST[nombre]','$_POST[apellido]','$_POST[telefono]','$_POST[direccion]','$_POST[email]', '$fecha', '$_POST[centro_vot]', '$_POST[sexo]'";
				$crud->crear();
				$crud = null;
		...
		case 'grabar_conformante':
			$crud->sql = "SELECT id from caracterizacion where cedula = $_POST[ced_resp] and id_institucion = $_POST[id_institucion]";
			$crud->total();
			if($crud->total > 0)
			{
			...
			else
			{
				...
				$crud->campos = "id_institucion_lider, cedula ,nacionalidad ,  nombre ,  genero ,  fechanacimiento ,  telefono1  , sector, calle ,  estado ,  municipio ,  parroquia ,  email ,  twitter, vota";
				$crud->valores = "$id_institucion,$ced_resp,'$nac_resp' ,'$nom_resp' ,'$genero','$fecha' ,  '$tlf1' ,  '$sectorr', '$callee', $_POST[estado] ,  $_POST[municipio] ,  $parroquia ,  '$email' ,  '$twitter', '$vota'";
				$crud->crear();

				$crud->from = "caracterizacion_datos_al";
				$crud->campos = "id_institucion_lider, cedula, nivel, profesion, ocupacion, estudia, institucion, carrera, trabaja, sitio, publico, privado, independiente";
				$crud->valores = "$id_institucion,$ced_resp,'$titulo', '$profesion', '$ocupacion', '$estudia', '$institucion', '$carrera', '$trabaja', '$sitio','$publico', '$privado', '$independiente'";
				$crud->crear();

				$crud->from = "caracterizacion_militancia";
				$crud->campos = "id_institucion_lider,cedula, tiempo, tareas_partido, clp, ubch, congreso_patria, delegado_patria, gpp, ms, mr, mri, ffm, consejoc, n_consejo, movimientos, viviendo_venezolanos, movimiento_campe, movimiento_cultu, colectivo, n_colectivo";
				$crud->valores = "$id_institucion,$ced_resp, '$militancia', '$tareas', '$clp', '$ubcn', '$congreso_p', '$delegado_p', '$gpp', '$ms', '$mr', '$mri', '$ffm', '$consejoc', '$n_consejo','$movimiento_s','$viviendo_vene', '$movimiento_campe', '$movimiento_cultu', '$colectivo','$n_colectivo'";
				$crud->crear();
