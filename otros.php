	...
	<div class="container-fluid">
		<div class="col-md-12">
			<table class="table table-strepead table-hover" id="tabla">
				...
				<?php
					$crud->sql = "SELECT id, nombre, apellido, cedula, nac, telefono, email, direccion, fecha_nacimiento, sexo, centro_votacion FROM estructuras WHERE id_otros = $_SESSION[id]";
					$crud->leer();
                    if(count($crud->filas) > 0)
                    {
                        foreach ($crud->filas as $row) 
                        {
                            $boton = "<a href='#modificar_estructura' data-toggle='modal' class='btn btn-warning btn-sm' title='editar'
                                        ...
                                        data-sexo ='$row[sexo]'
                                        data-centro ='$row[centro_votacion]'
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header login-header">
             ...
             <form class="form-horizontal" id="form-modificar" action="grabar_otros.php" method="POST">
             <input type="hidden" name="accion" value="modificar_estructura">
             <input type="hidden" name="id_modificar" id="id_modificar" value="">
             <input type="hidden" name="centro_vot_edit" id="centro_vot_edit">
                <div class="modal-body">
                    <div class="row">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                         ...
                         <div class="form-group">
                            ...
                            <div class="col-md-3">
                                <input type="text" name="sexo_edit" id="sexo_edit" required="" class="form-control">
                            </div>
                        </div>
                        <div class="col-md-offset-3 col-md-8 text-center">
                            <p  id="aviso_edit" class="label label-danger" style="color: white; font-weight: bold; font-size: 12px;"></p>
                        </div>
                    </div>
                </div>
            <div class="modal-footer">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header login-header">
             ...
             <form class="form-horizontal" id="form-agregar" action="grabar_otros.php" method="POST">
             <input type="hidden" name="accion" value="grabar_estructura">
             <input type="hidden" name="id_otros" value="<?php echo $_SESSION['id']; ?>">
...
<script type="text/javascript">
	$(function(){
        ...
        function rev_rep1(ced){
            $("#aviso_edit").html('');
            $("#aviso_edit").show('slow/400/fast');
            $("#aviso_edit").text('Cargando datos por favor espere...');
            $.get('http://consultaelectoral.bva.org.ve/cedula='+ced, function(data,textStatus) {
                if(data.length > 0)
                {
                    $("#aviso_edit").html('');
                    $("#nombre_edit").val(data[0].p_nombre);
                    $("#apellido_edit").val(data[0].p_apellido);
                    $("#fecha_nac_edit").val(data[0].f_nac);
                    $("#centro_vot_edit").val(data[0].cod_nuevo);
                    $("#sexo_edit").val(data[0].sexo);
                }        
            });
            setTimeout(function(){
            $("#aviso_edit").hide('slow/400/fast');
            }, 3000);
        };
        ...
        $("#modificar_estructura").on('show.bs.modal', function(e){
            ...
            var x = $(e.relatedTarget).data().sexo;
                    $(e.currentTarget).find("#sexo_edit").val(x);
            var x = $(e.relatedTarget).data().centro;
                    $(e.currentTarget).find("#centro_vot_edit").val(x);
        });
    //----------------------------SUBMITS-----------------------------------------
        ...
        $("#form-agregar").submit(function(){
            ...
            <?php 
                $crud->sql = "SELECT nombre, apellido, 
                                (SELECT count(*) from caracterizacion where id_otro_lider = estructuras.id) as total FROM estructuras WHERE id_otros = $_SESSION[id]";
                $crud->leer();
                if(count($crud->filas) > 0)
                {
                    foreach ($crud->filas as $row) 
                    {
                        ?>
                            {
                            "category": "<?php echo $row['nombre']." ".$row['apellido']; ?>",
