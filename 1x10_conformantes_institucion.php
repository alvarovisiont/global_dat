<?php
	...
	$id = base64_decode($_GET['id_institucion']);
	$crud->sql = "SELECT id from caracterizacion where id_institucion_lider = ".$id;
	$crud->total();
	$total = $crud->total;
?>
<div class="row">
						...
						<?php
							$crud->sql = "SELECT * from caracterizacion 
										...
										INNER JOIN caracterizacion_militancia cm ON caracterizacion.cedula = cm.cedula  
										 where caracterizacion.id_institucion_lider = $id and caracterizacion_datos_al.id_institucion_lider = $id and cm.id_institucion_lider = $id";
							$crud->leer();
	...
	<div id="agregar_modal" class="modal fade" role="dialog">
	    <div class="modal-dialog" style="width: 70%">
	        <div class="modal-content">
	            ...
	            <form class="form-horizontal" id="form_estructura" action="grabar_institucion.php" method="POST">
	                <input type="hidden" name="accion" value="grabar_conformante">
	                <input type="hidden" name="id_institucion" value="<?php echo $id; ?>">
	...
	//--------------------------------------------------------------------------------------
		...
		$("#form_estructura").submit(function(){
		 ...
		 var chart;
            ...
            var chartData = [
            <?php 
                $crud->sql = "SELECT nombre, apellido, (SELECT COUNT(*) from caracterizacion where id_institucion_lider = estructuras.id) as total from estructuras where id_institucion = $_SESSION[id]";
                $crud->leer();
                foreach ($crud->filas as $row) 
                {
